...
| **Automated Tests**           |           **100%**            | Vitest, `tests/`, 214 tests across 13 files, `pnpm test`. Real HTTP against a real spawned `next dev` server + a dedicated `proplity_test_db` (dropped/recreated per run) — see §6 below. Now run in CI on every PR (`.github/workflows/ci.yml`).                                                                                                                                                                                                                      |
