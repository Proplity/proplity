...
| **Automated Tests**           |           **100%**            | Vitest, `tests/`, 214 tests across 13 files, `pnpm test`. Real HTTP against a real spawned `next dev` server + a dedicated `proplity_test_db` (dropped/recreated per run) — see §6 below. Now run in CI on every PR (`.github/workflows/ci.yml`). Separately, a Playwright UI suite lives in `tests/e2e/` (`pnpm test:e2e`, 35 tests: smoke, per-role flows, responsive nav, featured-property popup) — run in CI as its own `E2E (Playwright)` job against a production build and a seeded Postgres. |
...
All 36 routes below were built phase-by-phase, each verified against the real dev server and seeded database (RBAC boundaries, business-rule enforcement, and cleanup of test data), with a full writeup per phase in `docs/development-history/phases/`. As of Phase 10, all 36 also have automated HTTP-level regression tests (§6 below):

| Phase                            | Routes                                                                                                                            | Doc                                                       |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 0                                | `lib/api/withAuth.ts`, `pagination.ts`, `errors.ts`, `validate.ts` (shared infra, no routes)                                      | `docs/development-history/phases/domain-api-phase-0-*.md` |
...
| 10 — Automated Test Suite        | No new routes — 126 tests added covering all 36 above (214 today, after later phases)                                             | `domain-api-phase-10-*.md` (8 sub-phase docs)             |
...
- **214 tests across 13 files** (`tests/api/`, roughly one per domain: `auth`, `properties`, `maintenance`, `leases`, `financial`, `access-control`, `communications`, `vendors-and-admin`, `deferred-flows`, `orphaned-models`, `csv-excel-import-export`, `e-signature`, `health`); the original 126 were built across 8 sub-phases (Phase 10, `docs/development-history/phases/domain-api-phase-10-*.md`) closing Finding 3 of `docs/development-history/next-phase-analysis.md`. Run with `pnpm test`.
...
- `docs/development-history/phases/*.md` — one detailed writeup per completed phase (what/why/verification), including the 6 Phase 9 and 8 Phase 10 sub-phase docs.
