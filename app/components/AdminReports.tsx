...
import { TrendingUp, TrendingDown, DollarSign, Users, Building2, Wrench, Calendar, ChevronDown, BarChart3, RefreshCw } from 'lucide-react';
...
const ROLE_COLORS: Record<string, string> = { ADMIN: '#6b7280', MANAGER: '#3b82f6', LANDLORD: '#8b5cf6', TENANT: '#10b981', VENDOR: '#f59e0b' };
...
function KpiCard({ label, value, sub, icon: Icon, iconBg, iconColor }: { label: string; value: string; sub?: string; icon: React.ElementType; iconBg: string; iconColor: string }) {
  return (
...
export function AdminReports() {
  ...
  const cancelledRate = invoices.length > 0 ? (cancelledInvoices.length / invoices.length) * 100 : 0;
  ...
  const resolutionDays = completedMaintenance.filter((r) => r.completedAt).map((r) => (new Date(r.completedAt!).getTime() - new Date(r.createdAt).getTime()) / MS_PER_DAY);
  const avgResolutionDays = resolutionDays.length > 0 ? resolutionDays.reduce((s, d) => s + d, 0) / resolutionDays.length : 0;
  ...
  const newUsersByMonth = months.map(({ key, label }) => {
    ...
    for (const role of ['MANAGER', 'LANDLORD', 'TENANT', 'VENDOR']) {
      row[role.toLowerCase()] = users.filter((u) => u.role === role && monthKey(u.createdAt) === key).length;
    }
    return row;
  });
  const maintenanceByMonth = months.map(({ key, label }) => ({
    ...
    resolved: maintenanceRequests.filter((r) => r.completedAt && monthKey(r.completedAt) === key).length,
  }));
  ...
  const usersByRole = ['ADMIN', 'MANAGER', 'LANDLORD', 'TENANT', 'VENDOR'].map((role) => ({ name: role, value: users.filter((u) => u.role === role).length, fill: ROLE_COLORS[role] }));

  const units = properties.flatMap((p) => p.units);
  const occupiedUnits = units.filter((u) => u.status === 'OCCUPIED').length;
  const avgOccupancy = units.length > 0 ? (occupiedUnits / units.length) * 100 : 0;
  const avgRent = units.length > 0 ? units.reduce((sum, u) => sum + u.rentAmount, 0) / units.length : 0;
  ...
  const stateDetail = Object.entries(
    properties.reduce<Record<string, { total: number; approved: number; pending: number; units: typeof units }>>((acc, p) => {
      const state = p.state ?? 'Unknown';
    ...
    state,
    ...s,
    avgRent: s.units.length > 0 ? s.units.reduce((sum, u) => sum + u.rentAmount, 0) / s.units.length : 0,
    occupancy: s.units.length > 0 ? (s.units.filter((u) => u.status === 'OCCUPIED').length / s.units.length) * 100 : 0,
  }));
  ...
  const statusByRole = ['ADMIN', 'MANAGER', 'LANDLORD', 'TENANT', 'VENDOR'].map((role) => {
    ...
    return {
      ...
      other: roleUsers.filter((u) => u.status !== 'ACTIVE' && u.status !== 'PENDING_VERIFICATION').length,
    };
  });
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <button onClick={() => setShowPeriodMenu((v) => !v)} className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-50">
            ...
            {showPeriodMenu && (
              <>
                ...
                <div className="absolute top-10 right-0 z-20 min-w-[170px] rounded-xl border border-gray-200 bg-white py-1.5 shadow-lg">
                  {periods.map((p) => (
                    <button key={p} onClick={() => { setPeriod(p); setShowPeriodMenu(false); }} className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-gray-50 ${p === period ? 'font-medium text-blue-600' : 'text-gray-700'}`}>
                      {p}
                    </button>
                  ))}
                </div>
      ...
      <div className="flex w-fit gap-1 rounded-xl bg-gray-100 p-1">
        {TABS.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={revenueTrend}>
                  ...
                  <Line type="monotone" dataKey="billed" stroke="#818cf8" strokeWidth={2} strokeDasharray="4 3" dot={false} name="Billed" />
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={newUsersByMonth} barSize={10}>
                  ...
                  <Tooltip />
                  <Bar dataKey="tenant" fill={ROLE_COLORS.TENANT} name="Tenants" radius={[2, 2, 0, 0]} />
                  <Bar dataKey="manager" fill={ROLE_COLORS.MANAGER} name="Managers" radius={[2, 2, 0, 0]} />
                  <Bar dataKey="landlord" fill={ROLE_COLORS.LANDLORD} name="Landlords" radius={[2, 2, 0, 0]} />
                  <Bar dataKey="vendor" fill={ROLE_COLORS.VENDOR} name="Vendors" radius={[2, 2, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {['Month', 'Revenue Collected', 'New Users', 'Maintenance Resolved'].map((col) => (
                      <th key={col} className="border-b border-gray-200 px-4 py-3 text-left text-xs font-semibold tracking-wide whitespace-nowrap text-gray-500 uppercase">{col}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {revenueTrend.map((row, i) => (
                    <tr key={i} className="transition-colors hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        {(newUsersByMonth[i].manager as number) + (newUsersByMonth[i].landlord as number) + (newUsersByMonth[i].tenant as number) + (newUsersByMonth[i].vendor as number)}
                      </td>
                  ...
                  {allPayments
                    ...
                    .map(({ invoice, payment }) => (
                      <tr key={payment.id} className="transition-colors hover:bg-gray-50">
                        <td className="px-4 py-3 font-medium text-gray-800">{invoice.lease?.tenant.name ?? 'Unknown'}</td>
                        <td className="px-4 py-3 text-gray-700">{invoice.lease?.unit.property.name ?? '—'}</td>
                        <td className="px-4 py-3 font-semibold text-gray-900">{fmt(payment.amount)}</td>
                        <td className="px-4 py-3 text-gray-700">{invoice.type}</td>
                        <td className="px-4 py-3 text-gray-700">{new Date(payment.paidAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  {allPayments.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-gray-400">No payments recorded yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={newUsersByMonth}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="manager" stroke={ROLE_COLORS.MANAGER} strokeWidth={2} dot={false} name="Managers" />
                  <Line type="monotone" dataKey="landlord" stroke={ROLE_COLORS.LANDLORD} strokeWidth={2} dot={false} name="Landlords" />
                  <Line type="monotone" dataKey="vendor" stroke={ROLE_COLORS.VENDOR} strokeWidth={2} dot={false} name="Vendors" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <RechartsPie>
                  <Pie data={usersByRole} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, value }) => (value > 0 ? `${name} (${value})` : '')} labelLine={false}>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              {propertiesByState.length > 0 ? (
                <ResponsiveContainer width="100%" height={240}>
                  <RechartsPie>
                    <Pie data={propertiesByState} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, value }) => `${name} (${value})`} labelLine={false}>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart
                  data={['RESIDENTIAL', 'COMMERCIAL', 'INDUSTRIAL', 'MIXED_USE'].map((type) => ({ type, count: properties.filter((p) => p.type === type).length }))}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          ...
          <div className="rounded-xl border border-gray-200 bg-white">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {['State', 'Total Listings', 'Approved', 'Pending', 'Avg. Rent', 'Avg. Occupancy'].map((col) => (
                      <th key={col} className="border-b border-gray-200 px-4 py-3 text-left text-xs font-semibold tracking-wide whitespace-nowrap text-gray-500 uppercase">{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 w-20 overflow-hidden rounded-full bg-gray-200">
                            <div className="h-full rounded-full bg-blue-500" style={{ width: `${row.occupancy}%` }} />
                          </div>
                  ...
                  {stateDetail.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-gray-400">No properties yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
