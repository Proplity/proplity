  ...
  const renderSummaryCards = () => {
    ...
    if (breakdownType === 'properties') {
      ...
      const flagged = properties.filter(
        (p) => p.moderationStatus === 'FLAGGED' || p.moderationStatus === 'REJECTED',
      ).length;
      return (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map((u) => (
              <tr key={u.id} className="transition-colors hover:bg-gray-50">
                ...
                <td className="px-4 py-3.5 whitespace-nowrap">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-medium ${u.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : u.status === 'SUSPENDED' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}
                  >
                    {u.status}
                  </span>
                </td>
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map((r) => (
              <tr key={r.id} className="transition-colors hover:bg-gray-50">
                ...
                <td className="px-4 py-3.5 whitespace-nowrap">
                  <span
                    className={`flex items-center gap-1.5 text-xs font-medium ${r.status === 'IN_PROGRESS' ? 'text-blue-700' : 'text-orange-700'}`}
                  >
